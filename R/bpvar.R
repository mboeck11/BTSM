...
#' @usage bpvar(Data, plag=1,draws=5000,burnin=5000,prior="NG",SV=TRUE,h=0,thin=1,
...
bpvar<-function(Data,plag=1,draws=5000,burnin=5000,prior="NG",SV=TRUE,h=0,thin=1,hyperpara=NULL,eigen=FALSE,Ex=NULL,cons=FALSE,trend=FALSE,applyfun=NULL,cores=NULL,verbose=TRUE){
  ...
  #-------------------------- construct arglist ----------------------------------------------------#
  args <- .construct.arglist(bpvar)
  if(verbose){
    cat("\nStart estimation of Bayesian Panel Vector Autoregression.\n\n")
    cat(paste("Prior: ",ifelse(prior=="NG","Normal-Gamma prior",NULL)),".\n",sep="")
  ...
  globalpost <- .BPVAR_linear_wrapper(Yraw=xglobal,prior=prior,plag=plag,draws=draws,burnin=burnin,cons=cons,trend=trend,SV=SV,thin=thin,default_hyperpara=default_hyperpara,Ex=Ex,applyfun=applyfun,cores=cores)
  ...
  out  <- structure(list("args"=args,
                         ...
                         "store"=globalpost$store), class = "bpvar")
  end.bpvar <- Sys.time()
  diff.bpvar <- difftime(end.bpvar,start.bpvar,units="mins")
  mins.bpvar <- round(diff.bpvar,0); secs.bpvar <- round((diff.bpvar-floor(diff.bpvar))*60,0)
  if(verbose) cat(paste("\nNeeded time for estimation of bvar: ",mins.bpvar," ",ifelse(mins.bpvar==1,"min","mins")," ",secs.bpvar, " ",ifelse(secs.bpvar==1,"second.","seconds.\n"),sep=""))
  return(out)
}
...
print.bpvar<-function(x, ...){
  cat("---------------------------------------------------------------------------------------")
  cat("\n")
  cat("Model Info:")
  cat("\n")
  prior <- ifelse(x$args$prior=="NG","Normal Gamma Prior","not defined.")
...
#' @export
logLik.bpvar<-function(object, ..., quantile=.50){
