#' @name bivar
#' @title Bayesian Interacted Vector Autoregression
#' @usage bivar(Data, plag=1,draws=5000,burnin=5000,prior="NG",SV=TRUE,h=0,thin=1,
...
#' @param Idata Data for interaction term in matrix form.
...
bivar<-function(Data,Idata,plag=1,draws=5000,burnin=5000,prior="NG",SV=TRUE,h=0,thin=1,hyperpara=NULL,eigen=TRUE,Ex=NULL,cons=TRUE,trend=FALSE,applyfun=NULL,cores=NULL,verbose=TRUE){
  start.bivar <- Sys.time()
  #--------------------------------- checks  ------------------------------------------------------#
  if(!is.matrix(Data)){
    stop("Please provide the argument 'Data' either as 'matrix' object.")
  }
  if(!is.matrix(Idata)){
    stop("Please provide the argument 'Idata' either as 'matrix' object.")
  }
  if(!is.null(Ex)){
  ...
  if(verbose){
    cat("\nStart estimation of Bayesian Interacted Vector Autoregression.\n\n")
  ...
  if(is.matrix(Data)){
    ...
    if(isTS || isXTS){
      ...
      Idata      <- ts(coredata(Idata), start=c(as.numeric(years[1]),as.numeric(startmonth)),frequency=freq)
    }else{
      timeindex  <- seq.Date(from=as.Date("1830-08-01", format="%Y-%m-%d"), by="month", length.out = Traw)
      Data       <- ts(Data,  start=c(1830,8), frequency=12)
      Idata      <- ts(Idata, start=c(1830,8), frequency=12)
    }
    args$time <- timeindex
  ...
  default_hyperpara <- list(c=10, Multiplier=10, # hyperparameter setup for natural conjugate case
                            ...
                            tau0=0.1,tau1=3,kappa0=0.1,kappa1=3,p_i=0.5,q_ij=0.5,   # SSVS
                            e_lambda=0.01,d_lambda=0.01,a_start=0.7,sample_A=FALSE,a_log=TRUE,
                            use_R=FALSE) # NG
  paras     <- c("c","a_1","b_1","prmean","Bsigma_sv","a0","b0","bmu","Bmu","shrink1","shrink2","shrink3","shrink4",
                 "shrink4","tau0","tau1","kappa0","kappa1","p_i","q_ij","e_lambda","d_lambda","a_log","a_start","sample_A","use_R")
